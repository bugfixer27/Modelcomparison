...
## Sites
- **CAMPUS position** is user-supplied (40.8980559, −73.8995862, 53 m). It was built against a placeholder (40.8898, −73.9010, 50 m); the real position selects the *same* nearest grid point in every model, so no forecasts were re-extracted. The 3 m elevation change was applied to the stored campus MSLP by inverting the old reduction and re-reducing (+0.36 hPa), rather than re-fetching two years at one call per minute. ZentraCloud v4 exposes no device-location endpoint, so the position must stay in config.
